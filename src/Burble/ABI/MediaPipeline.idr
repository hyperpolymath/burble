...
-- public export
-- resample : (to : SampleRate) -> Stage from sr to sr
-- resample to (MkBuffer frame) =
--   -- In reality, this would perform interpolation/decimation.
--   -- The linear type ensures the original buffer is 'consumed'.
--   -- Note: AudioFrame size changes based on sample rate.
--   MkBuffer (resampleFrame frame)
-- where
--   resampleFrame : AudioFrame f s -> AudioFrame to s
--   -- Implementation omitted (stub)
