...
#
# The retired ZigBackend facade is deliberately unavailable. This task retains
# its comparison harness for historical reproducibility until proved SNIF guest
# benchmarks replace it; it currently reports BEAM reference timings only.
#
# Usage:
...
defmodule Mix.Tasks.Bench.Coprocessor do
  @moduledoc """
  Benchmark the current coprocessor reference path.

  Runs each kernel operation multiple times and reports average
  execution time. Do not use these results as SNIF performance evidence.
  """
  ...
  def run(_args) do
    ...
    Mix.shell().info("Retired direct Zig NIF available: #{zig_available}")
    Mix.shell().info("")
    ...
    pcm_binary =
      pcm |> Enum.map(fn s -> <<trunc(s * 32767)::signed-16>> end) |> IO.iodata_to_binary()
    ...
    Mix.shell().info(
      "    audit JSON: #{byte_size(audit_json)} bytes → #{byte_size(compressed_audit)} bytes (#{audit_ratio}x)"
    )
    ...
    {:ok, archive} =
      ElixirBackend.compress_audio_archive(
        for(_ <- 1..10, do: pcm),
        48_000,
        1
      )

    # 10 frames * 960 samples * 4 bytes
    raw_audio = 10 * @frame_size * 4
    archive_ratio = Float.round(raw_audio / byte_size(archive), 1)

    Mix.shell().info(
      "    audio archive: #{raw_audio} bytes → #{byte_size(archive)} bytes (#{archive_ratio}x)"
    )
