...
#
# Burble Coprocessor Benchmark — reference harness.
#
...
defmodule Burble.Coprocessor.BenchmarkTest do
  ...
  setup_all do
    # The retired direct-NIF facade must remain unavailable.
    if not ZigBackend.available?() do
      IO.warn("Direct NIF retired; benchmarking the BEAM reference only.")
    end
  ...
  describe "Audio Kernel Benchmarks" do
    ...
    test "Noise Gate SIMD speedup" do
      ...
      if ZigBackend.available?() do
        ...
        IO.puts(
          "[Benchmark] Noise Gate (500 frames): Elixir=#{elixir_time}µs, Zig=#{zig_time}µs, Speedup=#{Float.round(speedup, 2)}x"
        )
  ...
  describe "DSP Kernel Benchmarks" do
    test "FFT 1024-point speedup" do
      ...
      if ZigBackend.available?() do
        ...
        IO.puts(
          "[Benchmark] FFT (100 runs): Elixir=#{elixir_time}µs, Zig=#{zig_time}µs, Speedup=#{Float.round(speedup, 2)}x"
        )
