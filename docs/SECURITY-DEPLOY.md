...
Before running `scripts/vps-bootstrap.sh` in production:

> `scripts/vps-bootstrap.sh` (added by PR #186) is the actual
> one-command self-host path: it detects the OS/hardware, asks
> public-internet vs. Tailscale-only, generates every secret this
> checklist requires, builds/starts the stack via
> `containers/selur-compose.toml`, and health-checks before declaring
> success. **It is not the same file as `./burble-launcher.sh`** at
> the repo root — that script is a local/desktop launcher generated by
> `launch-scaffolder` that starts the server with `MIX_ENV=dev`
> (`mix phx.server` on `localhost:6473`) for OS menu/dock integration.
> Running `burble-launcher.sh` "in production" would boot a dev-mode
> server; use `scripts/vps-bootstrap.sh` for production self-hosting.
