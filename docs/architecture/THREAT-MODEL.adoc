...
Burble is a voice-first, self-hostable, end-to-end encrypted communications platform designed for teams, communities, and embedded applications. The system supports four deployment topologies—monarchic (single-server), oligarchic (federated cluster), distributed (open mesh), and serverless (peer-to-peer)—each balancing scalability, moderation, and decentralization. The control plane runs on Elixir/Erlang OTP with per-room GenServer isolation. The media plane uses a WebRTC-compatible SFU (Selective Forwarding Unit) that *does not* decrypt E2EE media: voice frames pass through uninterpreted. Application-owned native code is not loaded into the BEAM. Pure numeric/buffer acceleration is permitted only through proved ReleaseSafe Zig guests behind SNIF/WASM; until those guest artifacts exist, Burble uses its BEAM reference kernels. Idris2 modules provide type-check-level ABI design assurance, while model-to-runtime conformance remains an explicit open obligation.
